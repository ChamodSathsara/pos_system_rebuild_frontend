...
function AcceptDialog({ transfer, onClose }: { transfer: StockTransfer; onClose: () => void }) {
  ...
  const submit = () => {
    if (transfer.lines.some((x) => !Number.isFinite(Number(qty[x.transferRequestLineId])) || Number(qty[x.transferRequestLineId]) < 0 || Number(qty[x.transferRequestLineId]) > x.requestedQty)) {
      toast.error("Approved quantities must be between zero and the requested quantity.");
      return;
    }
...
function DispatchDialog({ transfer, onClose }: { transfer: StockTransfer; onClose: () => void }) {
  ...
  const reviewDispatch = () => {
    if (!vehicleNo.trim() || !driverName.trim() || lines.some((x) => !x.batchId || !Number.isFinite(Number(x.quantity)) || Number(x.quantity) <= 0)) { toast.error("Vehicle, driver, batch and positive quantities are required."); return; }
