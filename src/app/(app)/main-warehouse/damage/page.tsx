...
export default function CentralDamagePage() {
  ...
  const save = () => { const value = form.getValues(); if (!value.reason.trim()) { form.setError("reason", { message: "Reason is required." }); return; } mutation.mutate({ itemCode: value.itemCode, branchCode: null, warehouseCode: code, quantity: Number(value.quantity), costAmount: null, reason: value.reason.trim(), damageDate: value.damageDate || null }, { onSuccess: () => { setConfirm(false); setOpen(false); form.reset({ itemCode: "", quantity: "", reason: "", damageDate: today() }); } }); };
