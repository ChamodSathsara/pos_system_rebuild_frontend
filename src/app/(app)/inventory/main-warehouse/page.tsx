...
export default function CentralWarehouseDashboard() {
  ...
  const metrics = [["Total Items", stocks.length], ["Total Quantity", stocks.reduce((sum, item) => sum + item.currentQty, 0)], ["Low Stock", lowStock.data?.length ?? 0], ["Recent Damage", recentDamage.length], ["Pending Requests", requests.filter((item) => item.status === "Submitted").length], ["Dispatched Transfers", requests.filter((item) => ["Dispatched", "Received"].includes(item.status)).length], ["Awaiting Branch Receipts", requests.filter((item) => item.status === "Dispatched").length]] as const;
